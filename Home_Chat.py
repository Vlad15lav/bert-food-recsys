...
BOT_MESSAGE_TEMP = (
    ...
    "Хотел бы поделиться с вами несколькими интересными рецептами, например...\n\n",
    "Позвольте порекомендовать вам некоторые восхитительные рецепты, включая...\n\n",
    "С радостью предложу вам набор рецептов, в том числе...\n\n",
    "Рекомендую присмотреться к таким интересным рецептам:\n\n",
    "Предлагаю рассмотреть следующие рецепты, которые обязательно стоит попробовать:\n\n",
    "Могу порекомендовать несколько вкусных рецептов, среди которых...\n\n",
    "Вот несколько замечательных рецептов, которые рекомендую попробовать:\n\n",
...
def main():
    ...
    with st.expander("Подробнее о приложении"):
        st.write("""AI Chef - это чат-бот, разработанный для предоставления персонализированных
    рекомендаций по приготовлению разнообразных блюд. Благодаря использованию модели машинного обучения
    и анализу предпочтений пользователей, ассистент предлагает уникальные рецепты, соответствующие вкусам
    ...
    if "messages" not in st.session_state:
        ...
        start_message = "Привет! Я – AI Chef. С моей помощью вы можете "\
                "удобнее и быстрее подобрать для себя рецепт блюда. Присылайте ваши пожелания, "\
                "а я порекомендую подходящий рецепт. Примеры запросов: «Предложи что-нибудь из помидоров, "\
    ...
    if prompt := st.chat_input("Напиши что нибудь"):
        ...
        rating_arg = get_tfidf_recomendation(prompt, tfidf_model, tfidf_embed_items)
        response = np.random.choice(BOT_MESSAGE_TEMP)
        for i in range(3):
            item_id = int(rating_arg[i])

            response += "Название: {}\nСостав: {}\n{}. Время готовки: {}".format(*data_items.loc[[item_id],
                ['name', 'ingredient', 'type_kitchen', 'time_cook']].values.reshape(-1))
            response += f"\n\nПодробнее рецепт по [ссылке]({data_items.loc[item_id, 'link']})\n\n"
